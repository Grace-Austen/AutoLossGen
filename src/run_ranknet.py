...
def main():
	...
	parser.add_argument('--model_name', type=str, default='RankNet', help='Choose model to run.')
	parser.add_argument('--model_path', type=str, help='Model save path.',
						default=os.path.join(MODEL_DIR, 'rankNet.pt'))  # '%s/%s.pt' % (model_name, model_name)))
	...
	model = RankNet(user_num=data_loader.user_num, item_num=data_loader.item_num,
	...
	if torch.cuda.device_count() > 0:
		...
		model = model.cuda()
	data_processor = DataProcessor(data_loader, model, rank=True, test_neg_n=args.test_neg_n)
